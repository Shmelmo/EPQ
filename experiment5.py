...
class SunDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        val = torch.Tensor([np.double(self.data[idx]),
                            np.double(self.data[idx + self.offset2]),
                            np.double(self.data[idx + self.offset])])
        return (images, val)
...
for epoch in range(30):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        # print statistics
        running_loss += np.array(loss.item())
...
##            running_loss = 0.0
    ...
    with torch.no_grad():
        for datax in testloader:
            ...
            total += np.array(measure.item())
    print("[epoch: %d] loss: %.5f day ahead: %.5f week ahead: %.5f, testset loss: %.5f day ahead: %.5f week ahead %.5f" % (epoch + 1,
                                                          running_loss[0] / (len(trainloader)*np.var(np.array(trainset.data[:-7]))),
                                                          running_loss[1] / (len(trainloader)*np.var(np.array(trainset.data[1:-6]))),
                                                          running_loss[2] / (len(trainloader)*np.var(np.array(trainset.data[7:]))),
                                                          total[0] / (len(testloader) * np.var(np.array(testset.data[:-7]))),
                                                          total[1] / (len(testloader) * np.var(np.array(testset.data[1:-6]))),
                                                          total[2] / (len(testloader) * np.var(np.array(testset.data[7:])))
                                                          ))
