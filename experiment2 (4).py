...
for epoch in range(30):  # loop over the dataset multiple times
    ...
    ##            running_loss = 0.0
            ...
            PATH = "./solar_net_%s/solar_epoch_%2d_offset_%ddays.pth" % (job_id, epoch + 1, trainsets[j].offset)
    ...
    print("[epoch: %d] loss: %.5f day ahead: %.5f week ahead: %.5f, testset loss: %.5f day ahead: %.5f week ahead %.5f" % (epoch + 1,
                                                          ...
                                                          total[1] / (len(testloaders[1]) * np.var(np.array(testsets[1].data[1:-6]))),
                                                          total[2] / (len(testloaders[2]) * np.var(np.array(testsets[2].data[7:])))
                                                          ))
    if not os.path.isdir("./solar_net_{}".format(job_id)):
        os.mkdir("./solar_net_%s" % (job_id))
