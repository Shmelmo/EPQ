...
class SunDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        val = torch.Tensor(([np.double(self.data[idx])],
                           [np.double(self.data[idx + 1])],
                           [np.double(self.data[idx + 7])]))
        return (images, val)
...
for epoch in range(30):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(trainloaders, 0):
        ...
        for j in range(len(nets)):
            ...
            loss[j] = criterion(outputs, targets[j])
            loss[j].backward()
            optimizers[j].step()
    ...
    ##        running_loss = 0.0
        ...
        # line.set_data(x,y)
        # ax.autoscale_view())
            if not os.path.isdir("./solar_net_%s" % (job_id)):
                os.mkdir("./solar_net_%s" % (job_id))
            PATH = "./solar_net_%s/solar_epoch_%2d_offset_%ddays.pth" % (job_id, epoch + 1, j)
            torch.save(nets[j].state_dict(), PATH)
    
    total = np.zeros((len(nets)))
    measure = [0, 0, 0]
    with torch.no_grad():
        for datax in testloaders:
            images, targets = datax[0].to(device), datax[1]
            targets = targets.numpy()
            targets = np.transpose(targets, (1, 0, 2))
            targets = torch.Tensor(targets)
            targets = targets.to(device)
            for i in range(len(nets)):
                outs = nets[i](images)
                predicted = outs.data
                measure[i] = criterion(predicted, targets[i])
                total[i] += measure[i].item()
    print("[epoch: %d] loss: %.5f day ahead: %.5f week ahead: %.5f, testset loss: %.5f day ahead: %.5f week ahead %.5f" % (epoch + 1,
                                                          running_loss[0] / (len(trainloaders)*np.var(np.array(trainsets.data[:-7]))),
                                                          running_loss[1] /                                              (len(trainloaders)*np.var(np.array(trainsets.data[1:-6]))),
                                                          running_loss[2] / (len(trainloaders)*np.var(np.array(trainsets.data[7:]))),
                                                          total[0] / (len(testloaders) * np.var(np.array(testsets.data[:-7]))),
                                                          total[1] / (len(testloaders) * np.var(np.array(testsets.data[1:-6]))),
                                                          total[2] / (len(testloaders) * np.var(np.array(testsets.data[7:])))
                                                          ))
